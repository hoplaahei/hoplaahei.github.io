...
When `rsnapshot` is configured right, making a complete backup of the OS is as simple as running `rsnapshot manual` (I prefer this to an automatic cron job). The initial backup is a little slow and takes up as much disk space as you've used on the root filesystem, but only the changes get backed up on subsequent backups. `rsnapshot` is great for reverting a file system "back-in-time" to different states.
...
The relevant parts of the config I use are below. The config is easily modified to work without LVM by commenting out the LVM stuff, and changing the first parameter of `backup` to a standard partition e.g., `/dev/sda2`. However, without the LVM snapshotting capability, I'd recommend first unmounting `/` and booting into another OS or LiveCD to do the backup. Otherwise, running processes might only partially complete writing to files, or some file changes might not register on the destination if they happen after the file is already synced over, leading to an inconsistent backup.

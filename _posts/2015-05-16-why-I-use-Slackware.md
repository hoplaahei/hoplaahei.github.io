...
The package build scripts of some other distros are complex, and overuse syntactic sugar; this sugar may help package maintainers who need to automate looking after large quantities of packages, but it also adds unecessary layers of abstraction and complexity for the average user. Conversely, Slackbuilds use simple, but well documented shell script. Variables listed at the start of Slackbuilds make it easy for the user to discover and set the interesting features of software.
...
Also, the full base install of Slackware includes many of the useful command-line tools and development tools that end up getting installed gradually on typical systems anyway. The 'Full' Slackware install is recommended, as it includes many of the development files needed for building and installing software from Slackbuilds. On the rare occasion when no Slackbuild is available, `src2pkg` helps make Slack packages from any source.
